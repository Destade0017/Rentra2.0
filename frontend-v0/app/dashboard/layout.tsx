  ...
  return (
    <AuthGuard>
      <div className="flex h-screen bg-[#f8fafc]">
        {/* Sidebar (Desktop) */}
        <aside className="hidden lg:flex flex-col w-[280px] bg-white border-r border-slate-200/60 h-full">
          {/* Brand Header */}
          <div className="p-10">
                ...
                <div className="space-y-1">
                  {group.items.map((item) => (
                    <Link key={item.href} href={item.href} className="block group">
                      ...
                      >
                        <div className={`transition-all duration-300 ${isActive(item.href) ? 'text-indigo-600 scale-110' : 'text-slate-300 group-hover:text-slate-900'}`}>
                          {item.icon}
                        </div>
